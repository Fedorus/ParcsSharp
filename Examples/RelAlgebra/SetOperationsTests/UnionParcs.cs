...
{
    public class UnionParcs
    {
        private const int ItemsNumber = 1_000_000;
        private const int RelPointNumber = 4;
        ...
        static void TestOneThreaded()
        {
            ...
            operations.Union(list2.Select(x=>x.Slice.AccessBackingBytes(0).Array));
        ...
        public static async Task TestBetaFiltrationFileAsync(List<string> daemonsUrls)
        {
            ...
            command.DataTo = resPoint.Channel;
            await r1.SendAsync(command );
            
            //send 2nd point
            await r2.SendAsync(new RelCommand()
                {CommandType = CommandType.Union, DataFrom = r1.Channel, DataTo = resPoint.Channel});
            
            //send result point
            var command2 = new RelCommand() {DataFrom = r1.Channel, DataTo = r2.Channel, CommandType = CommandType.ReceiveData};
        ...
        public static async Task RelPoint(PointInfo info)
        {
            ...
            do
            {
                ...
                if (command.CommandType == CommandType.Filter || command.CommandType == CommandType.Union || command.CommandType == CommandType.SendData)
                {
